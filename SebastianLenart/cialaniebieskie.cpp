...
/* MODYFIKACJE:
 *  zrobie zderzenia ze statkiem i ew zmiana grafiki
 *  mniejsze satelity i lepsze rozmieszczenie aby sie nie nakladaly
 *  zrobie rotacje satelity na posdtawie listy punktow z klasy Planety OKRAG i  przyrownanie licznika "i" jakos trzeba to wykombinowac
 *  powyzsze modyfikacja musi byc dobrze zsynchronizowana
 *  wolniejsye rotacje planet popryey mniejszenie kroku i a nie przez zmiane timera bo zle to wychodzis, ale ma to wplyw na wiele
 *  rzeczy wiec trzeba bedzie pomyslec!!!
 *
 */
...
//    qDebug()<<getPksSrSloncaOgolny();
//    qDebug()<<Ziemia->getPktPlanetyOgolny();
//    qDebug()<<SatelitaZiemi->getPktSatelityOgolny();
...
//        qDebug()<<it.key()->nameCiala<<it.value(); // jak odczytac wektor???
    ...
    //DODAJ DO MAPY ktorsa juz utworzylem!!!
